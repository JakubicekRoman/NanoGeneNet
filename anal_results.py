...
for i in range(50000,squiggle.size(1)-32,1000):
    ...
    # Dopredne
    squiggle = squiggle[:,i:-1:1,:]
    lbl = lbl[:,i:-1:1,:]   
    loc = loc-i
    loc[loc<0]=0
    
    # # Zpetne
    # squiggle = squiggle[:,0:squiggle.size(1)-i,:]
    # lbl = lbl[:,0:lbl.size(1)-i,:]  
    # loc[loc>squiggle.size(1)]=squiggle.size(1)
