...
class CNN(nn.Module):
    def __init__(self,numF,h_size,y_size):
        super(CNN, self).__init__()

        self.conv1 = nn.Conv1d(in_channels = 1, out_channels =  numF, kernel_size = 3, stride = 1, padding=1, padding_mode='replicate')
        self.conv2 = nn.Conv1d(in_channels = 16, out_channels =  numF, kernel_size = 3, stride = 2, padding=1, padding_mode='replicate')
        self.conv3 = nn.Conv1d(in_channels = 32, out_channels =  numF, kernel_size = 3, stride = 2, padding=1, padding_mode='replicate')
        self.conv4 = nn.Conv1d(in_channels = 64, out_channels =  numF, kernel_size = 3, stride = 2, padding=1, padding_mode='replicate')

        # self.linear1=nn.Linear(h_size+numF+1,int(h_size/4), bias=True)
        # self.linear1=nn.Linear(h_size+1, h_size, bias=True)
        
        self.linear1=nn.Linear(64,1024, bias=True)
